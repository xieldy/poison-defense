...
class GDPoisoner(object):
    def __init__(self, x, y, testx, testy, validx, validy, \
        ...
        self.samplenum = x.shape[0] #sample number
        self.feanum = x.shape[1] #feature number

        self.objective = objective #which objective to use [train, validation, norm difference]
        self.opty = opty #whether optize the y value
    ...
    def poison_data(self,poisx,poisy,tstart,visualize,newlogdir):
        ...
        poisct = poisx.shape[0] # the number of poison points
        print("Poison Count: {}".format(poisct))

        new_poisx = np.zeros(poisx.shape) # define the init poison points with zero matrix
        ...
        if self.mp: # if set the '-mp' option
            import multiprocessing as mp
            workerpool = mp.Pool(max(1, mp.cpu_count()//2 - 1))# define the process pool for multiprocessing
        else:
            workerpool = None
    ...
    def computeError(self, clf):
        ...
        w = np.reshape(clf.coef_, (self.feanum,))#coef is the coefficient of the fitted model 'clf'
        #numpy.reshape() is used to reshape the matrix but not change the value in it.
        sum_w = np.linalg.norm(w,1)
        #numpy.linalg.nrom() is used to calculate the norm of the matrix, '1' means calculete norm-L1
